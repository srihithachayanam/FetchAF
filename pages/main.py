...
# st.write("Debug: main.py is loaded")
...
def get_db_engine():
    try:
        ...
        db_status.error(f"Database connection error: {str(e)}")
        st.write("Debug: Check DB_USER, DB_PASSWORD, DB_HOST, DB_PORT, DB_NAME in .env")
        return None
...
def generate_sql_query(question, tables_info, timeout=15):
    ...
    if not api_key:
        st.error("Cohere API key not found in .env file. Please add COHERE_API_KEY=your_key to your .env file (without spaces around =).")
        st.write("Debug: .env file should be in the project root")
        return f'SELECT * FROM "{table_name}" LIMIT 10;'
...
Your query:"""
        ...
        st.error(f"Error from Cohere: {str(e)}")
        st.write("Debug: API Key used:", api_key[:4] + "..." + api_key[-4:])
        return f'SELECT * FROM "{table_name}" LIMIT 10;'
